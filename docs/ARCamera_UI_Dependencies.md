...
![Component Dependencies](diagrams/component_dependencies.png)

**主要コンポーネント:**

| コンポーネント | 役割 |
|---------------|------|
| CameraCaptureController | メインUI制御 (3,367行) |
| ARPhotoController | 写真/動画撮影 |
| RuntimeAvatarLoader | VRMロード |
| RuntimeFBXLoaderBridge | FBXロード |
| PoseSlotController | ポーズ制御 |
| VrmExpressionSetup | 表情制御 |
| LightingPanelController | ライティング/シャドウ設定 |
| ARPlaneVisibilityController | AR平面表示制御 |
...
![Capture Flow](diagrams/capture_flow.png)

**操作:**
- **タップ (0.5秒未満)**: 写真撮影 → フラッシュ演出 → サムネイル更新
- **長押し (0.5秒以上)**: 動画録画開始 → プログレスリング表示 → 最大5秒
