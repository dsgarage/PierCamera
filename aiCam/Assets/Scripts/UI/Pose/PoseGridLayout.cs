...
public class PoseGridLayout : MonoBehaviour
{
    ...
    [SerializeField] private List<PoseItem> items = new List<PoseItem>();
    ...
    public IReadOnlyList<PoseItem> Items => items;

    public void SetItems(IEnumerable<PoseItem> list, bool andRebuild = true)
    {
        items = new List<PoseItem>(list ?? Array.Empty<PoseItem>());
    ...
    public void RebuildLayoutFromScratch()
    {
        if (!contentRect)
        {
            Debug.LogWarning("[PoseGridLayout] contentRect is null.");
            return;
        }
    ...
    public void UpdateLayoutNow()
    {
        ...
        {
            Debug.LogWarning("[PoseGridLayout] contentRect or slotPrefab is null.");
            return;
        }
        ...
        {
            EnsureChildrenCount(items.Count); // 差分再利用モード
        }
    ...
    /// <summary>
    /// Content 直下の子を「確実に」全削除。Editor: Undo + Immediate / Play: 非表示→Destroy
    /// </summary>
    private void ClearAllSlotsInContentHard()
    {
        if (!contentRect) return;

        // 先にスナップショット（削除中に childCount 参照しない）
...
#endif
            {
                go.SetActive(false); // 同フレームでのチラつき防止
